...
void htInsert ( tHTable* ptrht, tKey key, tData data ) {
    ...
    if(item)
        ...
        item = (tHTItem *) malloc(sizeof(struct tHTItem));
        if(!item)   // Zla alokacia
            return;
...
void htDelete ( tHTable* ptrht, tKey key ) {
    ...
    if(!item)   //Ak je prazdny index
        return;

    if(!strcmp(item->key, key)){    //Kontrola zaciatku listu
