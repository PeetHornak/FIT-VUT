...
**/
    ...
    if(temp->Cont != Content){
        next = malloc(sizeof(struct tBTNode));
        if(!next){
            return;
        }
        next->Cont = Content;
...
**/
    ...
    tStackP *myStack = malloc(sizeof(tStackP));
    if(!myStack){
        return;
    }
    SInitP(myStack);
    ...
    while(!SEmptyP(myStack)){
        tBTNodePtr temp = STopPopP(myStack);
        if(temp->RPtr){
            Leftmost_Preorder(temp->RPtr, myStack);
        }
    }
    free(myStack);
}
...
**/
    ...
    while (!SEmptyP(myStack) || temp){
        if(temp){
            Leftmost_Inorder(temp, myStack);
        }
        temp = STopPopP(myStack);
        BTWorkOut(temp);
        temp = temp->RPtr;
    }
...
**/
    ...
    tStackP* myStackP = malloc(sizeof(tStackP));
    if(!myStackP){
        return;
    }
    tStackB* myStackB = malloc(sizeof(tStackB));
    if(!myStackB){
        return;
    }
    SInitP(myStackP);
    ...
    tBTNodePtr temp = RootPtr;
    Leftmost_Postorder(temp, myStackP, myStackB);
    while(!SEmptyP(myStackP)){
        temp = STopPopP(myStackP);

        if(STopPopB(myStackB)){
            SPushB(myStackB, false);
            SPushP(myStackP, temp);
            Leftmost_Postorder(temp->RPtr, myStackP, myStackB);
        }
        else{
            BTWorkOut(temp);
        }
...
**/
    ...
    while (!SEmptyP(myStack) || temp){
        ...
        tBTNodePtr myDelete = STopPopP(myStack);
        temp = myDelete->RPtr;
        free(myDelete);
    }
