...
/*  Úpravy: Kamil Jeøábek, záøí 2018                                          */
/* ************************************************************************** */
/*
...
*/
    ...
    if(q == NULL){                      // Kontrola prazdnej queue
        queueError(QERR_INIT);
        return;
    }
    for(int i = 0; i < QUEUE_SIZE; i++)     // Nastavenie kazdeho prvku na '*'
        q->arr[i] = '*'; 
...
*/
    ...
    *c = q->arr[q->f_index];    // Priradenie znaku do c
}
...
*/
    ...
    q->f_index = nextIndex(q->f_index);     // Posunutie prveho indexu
...
*/
    ...
    q->arr[q->b_index] = c;                 // Priradenie znaku na prvu volnu poziciu
    q->b_index = nextIndex(q->b_index);     // Posunutie volneho indexu
