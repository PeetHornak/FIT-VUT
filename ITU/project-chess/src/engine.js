...
var p4_log;
// if (this.imports !== undefined && this.printerr !== undefined) {
//   //seed or gjs
//   p4_log = function() {
...
var P4_USE_TYPED_ARRAYS = new Int32Array() !== undefined;
...
function p4_optimise_piece_list(state) {
  ...
  for (var colour = 0; colour < 2; colour++) {
    ...
    for (i = threats.length - 1; i >= 0; i--) {
      var mvs = threats[i];
      var scor = scores[mvs[2]];
      if (scor !== undefined) {
        var S = board[mvs[1]];
        var r = (1 + scor.piece > 3 + S < 4) * 0.01;
        if (scor.threatened < r) scor.threatened = r;
      }
    }
    var pieces2 = [];
    for (i = 20; i < 100; i++) {
...
function p4_make_move(state, s, e, promotion) {
  ...
  if (E || ep_taken) {
    ...
    for (i = 0; i < their_pieces.length; i++) {
      var theirPiece = their_pieces[i];
      if (theirPiece[1] != gone) {
        dest.push(theirPiece);
      }
    }
  }
...
// var P4_MOVE_CHECKMATE =
//   P4_MOVE_FLAG_OK | P4_MOVE_FLAG_CHECK | P4_MOVE_FLAG_MATE;
...
function p4_fen2state(fen, state) {
  ...
  if (fen_moveno === undefined) {
    ...
    fen_moveno = Math.max(
      1,
      parseInt(
        (32 - pieces) * 1.3 + (4 - fen_castles.length) * 1.5 + (mix - 16) / 5
      )
    );
...
function p4_zero_array() {
  if (P4_USE_TYPED_ARRAYS) return new Int32Array(120);
...
function p4_find_source_point(state, e, str) {
  ...
  if (possibilities.length == 0) {
    ...
    p4_log(
      "p4_find_source_point seems to have failed",
      state,
      e,
      str,
      possibilities
